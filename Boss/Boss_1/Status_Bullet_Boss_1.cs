...
public class Status_Bullet_Boss_1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Boss_ReceiveDame.currenthealth <= 0 || Boss_ReceiveDame.currenthealth <= 0)
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (Boss_1_ReceiveDame.currentHealth_Boss_1 > 0)
            {
                boss_receivedame.Player_ReceiveDame_Boss(data_enemy.Dame_Boss);
                Destroy(gameObject);
            }
            else if (Boss_1_ReceiveDame.currentHealth_Boss_1 <= 0)
            {
                return;
            }
        }
